...
const truck: VehicleDef = {
  ...
  parts: [
    { id: "cab", name: "Cab & driveline controls", group: "Chassis", hotspot: [0, 1.15, 1.05], description: "Driver station, telematics gateway and dash cluster feed.", sensorIds: ["speed", "gear", "steering"], setupIds: [] },
    { id: "engine", name: "Diesel engine", group: "Powertrain", hotspot: [0, 0.55, 1.35], description: "12L inline six with turbo and EGR. Watch coolant temp on long grades.", sensorIds: ["rpm", "throttle", "water", "oil", "fuel"], setupIds: ["fuelLoad"] },
    { id: "gearbox", name: "Automated gearbox", group: "Powertrain", hotspot: [0, 0.3, -0.9], description: "12-speed automated manual with retarder.", sensorIds: ["gear", "rpm"], setupIds: ["finalDrive"] },
    { id: "steer-axle", name: "Steer axle", group: "Mechanical", hotspot: [0.55, 0.3, 0.86], description: "Front axle, alignment and steering geometry.", sensorIds: ["lat-g", "steering", "tyre-temp-fl"], setupIds: ["camber", "tyrePressure"] },
    { id: "drive-axle", name: "Drive axle", group: "Mechanical", hotspot: [0.55, 0.3, -0.76], description: "Rear drive axle carrying the payload.", sensorIds: ["tyre-temp-rl", "tyre-wear-rl"], setupIds: ["payload", "tyrePressure"] },
    { id: "brakes", name: "Air brake system", group: "Braking", hotspot: [0.4, 0.3, 0.86], description: "Drum brakes with load-sensing valve and retarder assist.", sensorIds: ["brake", "brake-temp-fl", "brake-temp-rl"], setupIds: ["brakeBias"] },
    { id: "body", name: "Cargo body", group: "Chassis", hotspot: [0, 1.25, -0.6], description: "Box body carrying the payload; frontal area drives drag.", sensorIds: ["downforce", "speed"], setupIds: ["payload", "rideHeight"] },
  ],
...
const road: VehicleDef = {
  ...
  parts: [
    { id: "front-bumper", name: "Front bumper & splitter", group: "Aerodynamics", hotspot: [0, 0.35, 1.25], description: "Splitter and cooling apertures.", sensorIds: ["speed", "downforce"], setupIds: [] },
    { id: "engine", name: "Engine", group: "Powertrain", hotspot: [0, 0.55, 1.35], description: "Turbo four with direct injection.", sensorIds: ["rpm", "throttle", "water", "oil", "fuel"], setupIds: ["fuelLoad"] },
    { id: "gearbox", name: "Transmission", group: "Powertrain", hotspot: [0, 0.3, -0.9], description: "Six-speed automatic with torque converter.", sensorIds: ["gear", "rpm"], setupIds: ["finalDrive"] },
    { id: "front-suspension", name: "Front suspension", group: "Mechanical", hotspot: [0.38, 0.34, 0.64], description: "MacPherson struts with anti-roll bar.", sensorIds: ["lat-g", "steering"], setupIds: ["camber", "rideHeight"] },
    { id: "tyre-fl", name: "Front-left tyre", group: "Tyres", hotspot: [0.6, 0.3, 0.64], description: "Street compound; temperature window is much narrower than a slick.", sensorIds: ["tyre-temp-fl", "tyre-wear-fl", "brake-temp-fl"], setupIds: ["tyrePressure", "camber"] },
    { id: "tyre-rr", name: "Rear-right tyre", group: "Tyres", hotspot: [-0.6, 0.3, -0.88], description: "Rear contact patch under traction.", sensorIds: ["tyre-temp-rr", "tyre-wear-rr", "brake-temp-rr"], setupIds: ["tyrePressure"] },
    { id: "brakes", name: "Brake system", group: "Braking", hotspot: [0.4, 0.3, 0.86], description: "Vented steel discs; fade appears after repeated stops.", sensorIds: ["brake", "brake-temp-fl", "brake-temp-rr"], setupIds: ["brakeBias"] },
    { id: "cabin", name: "Cabin", group: "Chassis", hotspot: [0, 1.05, 0.1], description: "Occupant cell and instrument cluster.", sensorIds: ["speed", "lat-g"], setupIds: [] },
    { id: "rear-spoiler", name: "Rear spoiler", group: "Aerodynamics", hotspot: [0, 1.2, -1.2], description: "Lip spoiler trimming rear lift.", sensorIds: ["downforce"], setupIds: ["rearWing"] },
  ],
